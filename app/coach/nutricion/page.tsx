...
export default function CoachNutricion() {
  ...
  async function loadMealPlan(userId: string) {
    const supabase = createClient()
    const { data } = await supabase.from('meal_plans').select('meals').eq('user_id', userId).single()
    setMeals(data?.meals ? JSON.parse(JSON.stringify(data.meals)) : [])
  }
  ...
  function addMeal() {
    const updated = JSON.parse(JSON.stringify(meals))
    updated.push({ id: 'm' + Date.now(), name: 'Nueva comida', time: '12:00', items: [{ name: 'Alimento', cal: 0, p: 0, c: 0, f: 0, g: '100g' }] })
    setMeals(updated)
  }
    ...
    <div style={{ maxWidth: 860 }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 13, color: '#6B6762', marginTop: 4 }}>
          Edita el plan de cada atleta y ajusta sus macros
        </div>
      </div>
      ...
      {selectedUser && (
        <>
          ...
          {meals.map((meal, mealIdx) => {
            ...
            return (
              <div key={meal.id} style={{
                ...
                <div onClick={() => setExpandedMeal(isOpen ? null : meal.id)} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <input value={meal.name}
                      ...
                      style={{ fontSize: 15, fontWeight: 700, color: '#1A1916', background: 'transparent', border: 'none', outline: 'none', cursor: 'text', width: '100%', fontFamily: '-apple-system, sans-serif' }}
                    />
                  </div>
                  <input value={meal.time}
                    onChange={e => updateMealField(mealIdx, 'time', e.target.value)}
                    onClick={e => e.stopPropagation()}
                    style={{ fontSize: 12, color: '#9A9690', background: 'transparent', border: 'none', outline: 'none', width: 48, textAlign: 'center' as const, fontFamily: '-apple-system, sans-serif' }}
                  />
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#FF9F0A', minWidth: 70, textAlign: 'right' as const }}>
                    {mealTotal} kcal
                  </div>
                  <div onClick={e => { e.stopPropagation(); deleteMeal(mealIdx) }} style={{ padding: 4, cursor: 'pointer', opacity: 0.3 }}>
                    <X size={14} color="#FF453A" />
                  </div>
                  {isOpen ? <ChevronUp size={16} color="#9A9690" /> : <ChevronDown size={16} color="#9A9690" />}
                </div>

                {isOpen && (
                  <div style={{ padding: '8px 0' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 70px 50px 50px 50px 50px 32px', gap: 6, padding: '4px 18px 8px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                      {['Alimento', 'Cantidad', 'kcal', 'P', 'C', 'G', ''].map((h, i) => (
                        <div key={i} style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: 0.4, color: '#B0ACA8' }}>{h}</div>
                      ))}
                    </div>
                    {meal.items.map((item: any, itemIdx: number) => (
                      <div key={itemIdx} style={{ display: 'grid', gridTemplateColumns: '1fr 70px 50px 50px 50px 50px 32px', gap: 6, padding: '8px 18px', borderBottom: itemIdx < meal.items.length - 1 ? '1px solid rgba(0,0,0,0.04)' : 'none', alignItems: 'center' }}>
                        <input value={item.name} onChange={e => updateItem(mealIdx, itemIdx, 'name', e.target.value)}
                          style={{ fontSize: 13, color: '#1A1916', background: '#F5F2EE', border: 'none', borderRadius: 7, padding: '6px 10px', outline: 'none', width: '100%', fontFamily: '-apple-system, sans-serif' }} />
                        <input value={item.g} onChange={e => updateItem(mealIdx, itemIdx, 'g', e.target.value)}
                          style={{ fontSize: 12, color: '#6B6762', background: '#F5F2EE', border: 'none', borderRadius: 7, padding: '6px 8px', outline: 'none', width: '100%', textAlign: 'center' as const, fontFamily: '-apple-system, sans-serif' }} />
                        {['cal', 'p', 'c', 'f'].map(field => (
                          <input key={field} type="number" value={item[field]}
                            onChange={e => updateItem(mealIdx, itemIdx, field, e.target.value)}
                            style={{ fontSize: 12, color: '#1A1916', background: '#F5F2EE', border: 'none', borderRadius: 7, padding: '6px 4px', outline: 'none', width: '100%', textAlign: 'center' as const, fontFamily: '-apple-system, sans-serif' }} />
                        ))}
                        <div onClick={() => deleteItem(mealIdx, itemIdx)} style={{ cursor: 'pointer', display: 'flex', justifyContent: 'center', opacity: 0.3 }}>
                          <X size={14} color="#FF453A" />
                        </div>
                      </div>
                    ))}
                    <div onClick={() => addItem(mealIdx)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 18px', cursor: 'pointer', color: '#0A84FF', fontSize: 12, fontWeight: 600 }}>
          ...
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#FF9F0A'; (e.currentTarget as HTMLElement).style.color = '#FF9F0A' }}
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(0,0,0,0.15)'; (e.currentTarget as HTMLElement).style.color = '#9A9690' }}
          >
